...
namespace BestHTTP
{
    ...
    public static class HTTPManager
    {
        ...
        private static string GetKeyForRequest(HTTPRequest request)
        {
            ...
            {
            var tempProxyAddr = request.Proxy.Address;
            var tempCurrentURI = request.CurrentUri;
            return
#if !BESTHTTP_DISABLE_PROXY
                (request.Proxy != null ? new UriBuilder(tempProxyAddr.Scheme, tempProxyAddr.Host, tempProxyAddr.Port).Uri.ToString() : string.Empty) +
#endif
                                            new UriBuilder(tempCurrentURI.Scheme, tempCurrentURI.Host, tempCurrentURI.Port).Uri.ToString();
